...
<h1 align="center">멀티모델 AI 에이전트 오케스트레이션</h1>

<p align="center">
  Claude, GPT, Ollama 에이전트를 하나의 워크스페이스에서 함께 운영하세요.<br />
  역할마다 최적의 모델을 배치하고, 서로 협업시키세요.<br />
...
Octopal은 **멀티모델 AI 에이전트 오케스트레이터**입니다. 그룹 채팅 인터페이스에서 Claude, GPT, Ollama 에이전트를 나란히 배치하고, 각자 역할을 맡겨 실시간으로 협업시킵니다.

[**Goose**](https://github.com/block/goose)(by Block) 기반 오픈소스 프레임워크로, Agent Control Protocol(ACP)을 통해 각 에이전트를 적절한 프로바이더로 라우팅합니다. 특정 벤더에 종속되지 않습니다. 역할마다 최적의 모델을 골라 자유롭게 교체하세요.

모든 에이전트 데이터는 프로젝트의 `octopal-agents/` 폴더에 저장됩니다. 각 에이전트는 `config.json`과 `prompt.md`를 가진 서브폴더. 클라우드도, 계정도 필요 없습니다.

## 왜 멀티모델인가?

모델마다 잘하는 게 다릅니다. Octopal은 하나의 프로바이더에 모든 에이전트를 몰아넣는 대신, 역할마다 최적의 모델을 배치할 수 있게 합니다.

| 역할 | 모델 | 이유 |
|------|------|------|
| 코드 리뷰 | Claude Opus | 깊은 추론, 긴 컨텍스트 |
| 프론트엔드 개발 | GPT-4o | 빠른 반복, 넓은 지식 |
| 로컬 초안 작성 | Ollama (Llama) | 오프라인, 무료, 프라이빗 |
| 전략 기획 | GPT-5 | 구조화된 분석 |

같은 방에서 프로바이더를 섞어 쓰세요. 어떤 모델이 뒤에 있든 에이전트끼리 대화할 수 있습니다.

## 하이라이트

| | 기능 | 설명 |
|---|------|------|
| 🤖 | **멀티모델 오케스트레이션** | Claude, GPT, Ollama 에이전트를 같은 방에서 운영. 크로스모델 협업이 기본. |
| 🎯 | **에이전트별 모델 지정** | 에이전트마다 원하는 모델을 배치. 역할별로 프로바이더를 자유롭게 조합. |
| 🏠 | **로컬 모델 (Ollama)** | Ollama나 OpenAI 호환 로컬 서버 연결. 완전 오프라인, API 키 불필요. |
| 🧠 | **스마트 라우팅** | 히든 오케스트레이터가 컨텍스트를 읽고 적시에 적절한 에이전트를 호출. |
| 💬 | **그룹 채팅** | 에이전트들이 서로, 그리고 당신과 자연스럽게 대화. @멘션 또는 자동 라우팅. |
| 🔗 | **Agent-to-Agent** | 에이전트끼리 @멘션으로 연쇄 협업. 개입 없이도 알아서 동작. |
| 🐙 | **Octo Agents** | `octopal-agents/` 서브폴더로 에이전트 정의. 각 폴더가 독립 에이전트. |
| 📁 | **폴더 = 팀** | 폴더가 팀, 워크스페이스가 회사. 파일 정리하듯 에이전트를 조직. |
| 🔒 | **로컬 퍼스트** | 모든 것이 내 컴퓨터에서 실행. 클라우드 서버도 데이터 수집도 없음. |
